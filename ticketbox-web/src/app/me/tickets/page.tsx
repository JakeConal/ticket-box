...
export default function MyTicketsPage() {
  ...
  return (
    <main className={ui.page}>
      ...
      <section className="max-w-2xl border-b-4 border-ink pb-10">
        <div>
          <p className={ui.eyebrow}>Ticket wallet</p>
          <h1 className="mt-3 text-4xl font-black uppercase tracking-tighter sm:text-5xl">My tickets</h1>
      ...
      <section className="mt-8 grid gap-6">
        {groups.map((group) => (
          <div className={ui.panel} key={group.order.orderId}>
            <div className="flex flex-wrap items-center justify-between gap-3 border-b-2 border-ink pb-4">
              <h2 className="text-xl font-black uppercase tracking-tight">Order {shortId(group.order.orderId)}</h2>
        ...
        {!loading && groups.length === 0 ? (
          <section className={ui.emptyState}>
            <h2 className="text-xl font-black uppercase tracking-tight text-ink">No saved orders</h2>
